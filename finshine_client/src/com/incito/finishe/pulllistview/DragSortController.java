...
import com.incito.finishe.pulllistview.DragSortListView.ItemRemoveCallBack;
...
//    		LogUtil.d("MotionEvent e1 == null or e2 == null");
            ...
            if (mHitPos != MISS) {
                ...
                if (mDragInitMode == ON_DRAG && Math.abs(y2 - y1) > mTouchSlop && mSortEnabled) {
                    startDrag(mHitPos, deltaX, deltaY);
                		...
                		speedScale >= 2 && mRemoveEnabled) {
                    mIsRemoving = true;
                    startDrag(mFlingHitPos, deltaX, deltaY);
                }
            } else if (mFlingHitPos != MISS) {
                if (Math.abs(x2 - x1) > mTouchSlop&& speedScale >= 2 && mRemoveEnabled) {
                    mIsRemoving = true;
                    startDrag(mFlingHitPos, deltaX, deltaY);
    ...
    public void delMultItem(int start, int end, List<String> params){
		...
		// 这样做的原因：mDslv.getChildAt(index)方法中index范围似乎为0-(可见item个数-1)
    ...
    public void delAllItem(){
    	...
    	startItem = 0;
    	int size = mDslv.getCount() - mDslv.getHeaderViewsCount() - mDslv.getFooterViewsCount();
    ...
    private ItemRemoveCallBack removeCallBack =  new ItemRemoveCallBack() {
		...
		public void itemRemoveCallBack(int position) {
			if((position >= endItem || 
					position >= mDslv.getCount() - mDslv.getHeaderViewsCount() - mDslv.getFooterViewsCount() - 1)){
...
//				Log.d(NoticeFragment.NOTICE_TAG, "DragSortController 删除动画index:" + index);
